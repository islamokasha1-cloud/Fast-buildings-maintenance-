...
## 4-ب) فحصُ الصحّة بعد النشر (موصى به)

```bash
cd functions && node wa-health.js
```

تقريرٌ واحدٌ يراجع: مفاتيحَ `.env` · وجودَ القوالب المضبوطة معتمَدةً في WABA الصحيح ·
المشغّلاتِ المنشورةَ وبيئةَ `waSender` · مفتاحَ القتل وإخفاقاتِ الطابور · و**أيَّ دورٍ بلا
رقمٍ مُفعَّل**. **قراءةٌ محضة** — لا يكتب شيئاً، ويعيد `1` إن وجد خللاً.

> يمسك عطلين وقعا فعلاً: قالبٌ معتمَدٌ في **WABA خطأ**، و`.env` بلا
> `WHATSAPP_PHONE_NUMBER_ID` (النشرُ يمسحه من `waSender` فيتوقّف الإرسالُ كلُّه).
