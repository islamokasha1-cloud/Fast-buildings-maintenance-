...
/* ── حالةُ القراءة `seenBy`: كلٌّ يكتب مفتاحَه وحدَه ──
   ...
   جديد» — أي **يُخفي عنه تعليقاً أو تعديلاً في مهمّةٍ يخصّه** بلا أثرٍ ظاهر.
   وهذا لا يُحكَم عليه بقراءة السطر: القواعدُ تُقيَّم بـ«أو»، فالمحاكي وحدَه يُثبته. */
await seed(`${ST}/T9`, {
  title: "متابعة", status: "open", createdByUser: "رغده", assignedToUser: "خالد",
  shared: [], participants: ["رغده", "خالد"], comments: [], seenBy: { "رغده": 1000 }
});
await check("★ الطرفُ يسجّل أنّه قرأ المهمّة",
  assertSucceeds(updateDoc(doc(ASSIGNEE, `${ST}/T9`), { seenBy: { "رغده": 1000, "خالد": 2000 } })));
await check("★★★ ولا يكتب مفتاحَ زميله فيُطفئ عنه لونَ «فيها جديد» (إخفاءُ تعديلٍ يخصّه)",
  assertFails(updateDoc(doc(ASSIGNEE, `${ST}/T9`), { seenBy: { "رغده": 9000 } })));
await check("★★ ولا يمحو حالةَ قراءةِ زميله بكتابة الخريطة كاملةً من نسخةٍ قديمة",
  assertFails(updateDoc(doc(ASSIGNEE, `${ST}/T9`), { seenBy: { "خالد": 3000 } })));
await check("★★ وموظفٌ خارجَ الغرفة لا يكتب فيها حالةَ قراءةٍ أصلاً",
  assertFails(updateDoc(doc(OUTSIDER, `${ST}/T9`), { seenBy: { "سعيد": 1 } })));
await check("★ والتعديلُ الذي لا يمسّ seenBy يمرّ كما كان (الشرطُ لا يعرقل ما ليس شأنَه)",
  assertSucceeds(updateDoc(doc(ASSIGNEE, `${ST}/T9`), { title: "متابعة — معدَّلة", lastEditBy: "خالد" })));
