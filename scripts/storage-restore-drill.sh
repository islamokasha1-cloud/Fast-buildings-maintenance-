...
if [ "$FAIL" -eq 0 ] && [ "$SKIPPED" -eq 0 ]; then
  # ★ ختمُ البروفة: بلا أثرٍ مكتوب، يبقى «أعِدها كلَّ ٣ أشهر» رهينَ الذاكرة وحدَها —
  #   ولا سبيلَ لأحدٍ أن يعرف متى أُثبتت النسخةُ آخرَ مرّة. الختمُ يجعلها **رقابةً**
  #   يقرؤها backup-health.sh بدل أن تكون تذكيراً يُنسى.
  gcloud storage cp - "gs://$VAULT_BUCKET/_drills/storage-restore-$(date -u +%Y%m%dT%H%M%SZ).txt" \
    --project="$VAULT_PROJECT" >/dev/null 2>&1 <<EOF || true
بروفةُ استعادةِ الملفّات — ناجحة $PASS · ساقطة $FAIL
