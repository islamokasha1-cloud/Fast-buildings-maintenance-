...
(function(){
  ...
  // مطابقة مكوّن الذكاء باسمٍ في الكتالوج — فقط عند تطابق الوحدة يُربط السعر
  function matchCatalog(name, unit, source, priceKey){
    const nn = normName(name); if(!nn) return null;
    const u = normName(unit);
    // تطابق كامل بالاسم أولاً، ثم احتواء (بطول كافٍ لتجنّب مطابقات هشّة)
    let it = source.find(x=>normName(x.name)===nn);
    if(!it) it = source.find(x=>{ const xn=normName(x.name); return xn.length>=3 && (xn.includes(nn)||nn.includes(xn)); });
    if(!it) return null;
...
"- أرقام واقعية للسوق السعودي. أعِد JSON صالحاً فقط.";
      ...
      const out = await aiText([{role:"user",content:prompt}], {maxTokens:1600});
      let obj;
