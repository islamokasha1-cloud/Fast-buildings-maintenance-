...
//
// ── العهود المحروسة هنا ──
// • ض.ق.م تُحسب **على صافي السطر** (round(net×0.15)) — لا على سعر الوحدة ثم تُضرب
//   في الكمية (٣٫٥ × ٢٥٦ ⇒ ١٣٤٫٤٠ لا ١٣٥٫٦٨؛ تقريبُ الوحدة يضاعف خطأ نصف قرش بالكمية).
...
/* ═══ ١) الحساب عند الإنشاء — ض.ق.م ١٥٪ على صافي السطر ═══
   الاصطلاح المعتمد (v18.9.3215): الصافي = round(الوحدة × الكمية)، الضريبةُ = round(الصافي × ٠٫١٥)،
   والإجماليُّ جمعُهما. (لا: تقريبُ ضريبة الوحدة ثم ضربُها في الكمية — يضاعف خطأ التقريب.) */
L('\n=== ١) حساب البند عند الإنشاء (ض.ق.م ١٥٪) ═══');
const s1 = await page.evaluate(() => ({
  simple: _poRepriceItem({ unitCost: 100 }, 3),
  round: _poRepriceItem({ unitCost: 33.33 }, 7),
  half: _poRepriceItem({ unitCost: 3.5 }, 256),
...
check('١أ) ★ ض.ق.م الوحدة (عرض) ١٥ وإجمالي البند ٣٤٥', near(s1.simple.vatUnit, 15) && near(s1.simple.itemCost, 345),
  'vatUnit=' + s1.simple.vatUnit + ' itemCost=' + s1.simple.itemCost);
check('١أ) الضريبة = الصافي × ١٥٪ = ٤٥', near(s1.simple.vat, 45), 'vat=' + s1.simple.vat);
// 33.33 × 7 = 233.31 ⇒ ×15% = 34.9965 → 35.00 ⇒ الإجمالي 268.31
check('١ب) ★ التقريب على السطر: ٣٣٫٣٣ × ٧ ⇒ ٢٦٨٫٣١',
  near(s1.round.itemCost, 268.31), 'itemCost=' + s1.round.itemCost);
// ★★ الحادثة الأصل: 3.5 × 256 — تقريبُ الوحدة (0.525→0.53) كان يعطي 135.68 / 1031.68
check('١هـ) ★★ ض.ق.م على الصافي لا الوحدة: ٣٫٥ × ٢٥٦ ⇒ ضريبة ١٣٤٫٤٠ وإجمالي ١٠٣٠٫٤٠',
  near(s1.half.lineTotal, 896) && near(s1.half.vat, 134.4) && near(s1.half.itemCost, 1030.4),
  'net=' + s1.half.lineTotal + ' vat=' + s1.half.vat + ' itemCost=' + s1.half.itemCost);
