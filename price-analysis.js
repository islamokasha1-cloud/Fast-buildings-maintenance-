...
<body>
  ...
  // تطبيع كود الكتالوج للمقارنة الحرفية (إزالة المسافات + تصغير)
  function normCode(c){ return String(c==null?"":c).toLowerCase().replace(/\s+/g,"").trim(); }
...
"خامات الكتالوج:\n"+(mats||"(لا يوجد)")+"\n"+
"مصنعيات الكتالوج:\n"+(labs||"(لا يوجد)")+"\n"+
"- الربط بالكتالوج (عام لأي مكوّن): إن كان المكوّن مطابقاً فعلياً لأحد أصناف الكتالوج أعلاه (نفس المادة والنوع والصفة الجوهرية) فأعِد كوده الحرفي في حقل refCode كما هو تماماً بين القوسين []، وإلا اجعل refCode = null. لا تُطابق إلا عند التطابق الحقيقي — الصفة الجوهرية فارقة (مثال: أسمنت أبيض ≠ أسمنت أسود، بلاط مطفي ≠ بلاط لامع). عند الشك اجعل refCode = null.\n"+
...
"- أرقام واقعية للسوق السعودي. أعِد JSON صالحاً فقط.";
      ...
      const newMats = aiMats.map(function(m){
        ...
        const bc = findByCode(matSrc, m.refCode);
      ...
      const newLabs = aiLabs.map(function(l){
        ...
        const bc = findByCode(labSrc, l.refCode);
        if(bc && bc.rate!=null){
