...
(function(){
  ...
  let _editorSession = 0;   // يُرفع مع كل فتح للمحرّر — حارس ضد هبوط نتيجة AI في مسودة أخرى بعد await
  ...
  /* v18.9oj: المدموج يُحجب هنا — ونقطةٌ واحدة تكفي: matSource بوّابةُ أربعة
     مستهلكين (المنتقي، تحديث الأسعار، سياق الذكاء، المطابقة بالكود). و v18.9og
     أغلقت مواضع index.html وحدها ولم تكن ترى هذا الملف: البند المدموج كان يقف
     في منتقي الخامات كأنه حيّ، ويُحقن في سياق الذكاء فيأكل مقعداً ويُطابَق بكوده. */
  function isMerged(c){ try{ return !!(c && typeof c.mergedInto==='string' && c.mergedInto); }catch(e){ return false; } }
  ...
  function buildDOM(){
    ...
    <div class="card" style="margin-bottom:14px">
      <div class="card-header">
        <div class="card-title"><span class="ct-ico">${ICON("barChart")}</span> محرك تحليل الأسعار</div>
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
          <button class="btn btn-ghost btn-sm" data-viewer-hide onclick="window.priceAnalysis.exportExcel()" title="تصدير Excel">${ICON("download")} تصدير Excel</button>
          <button class="btn btn-primary btn-sm" onclick="window.priceAnalysis.openEditor()">${ICON("plus")} تحليل بند جديد</button>
        </div>
      </div>
      <div class="card-body">
...
<body>
  ...
  function paintStats(){
    const g = document.getElementById("pa-stat-grid");
    if(!g) return;
    const all = _items;
    const tot = all.length;
    function hasSrc(it,src){
      const comps = (it.materials||[]).concat(it.labor||[]);
      return comps.some(function(c){ return c && c.source===src; });
    }
    const priced = all.filter(function(c){ return c.unitPrice!=null && c.unitPrice>0; });
    const avg = priced.length ? priced.reduce(function(s,c){ return s+c.unitPrice; },0)/priced.length : 0;
    const linked = all.filter(function(it){ return hasSrc(it,"catalog"); }).length;
    const aiN = all.filter(function(it){ return hasSrc(it,"ai"); }).length;
    const avgTxt = isViewerMode() ? ICON("lock")
      : avg.toLocaleString("en-US",{minimumFractionDigits:0,maximumFractionDigits:0});
    g.innerHTML =
      '<div class="ast-stat total"><span class="si">'+ICON("barChart")+'</span><div><div class="sv">'+tot.toLocaleString("en-US")+'</div><div class="sl">بنود محلَّلة</div></div></div>'+
      '<div class="ast-stat"><span class="si">'+ICON("receipt")+'</span><div><div class="sv">'+avgTxt+'</div><div class="sl">متوسط سعر الوحدة (ر.س)</div></div></div>'+
      '<div class="ast-stat good"><span class="si">'+ICON("package")+'</span><div><div class="sv">'+linked.toLocaleString("en-US")+'</div><div class="sl">مرتبطة بالكتالوج</div></div></div>'+
      '<div class="ast-stat '+(aiN?"warn":"good")+'"><span class="si">'+ICON(aiN?"alertTriangle":"checkCircle")+'</span><div><div class="sv">'+aiN.toLocaleString("en-US")+'</div><div class="sl">تحوي مكوّنات استرشادية</div></div></div>';
  }
  ...
  function rowsHtml(kind){
    ...
    return arr.map(function(r,i){
      ...
      const badge = r.source==="catalog"
        ? `<span style="display:inline-flex;align-items:center;gap:3px;font-size:9px;background:#dbeafe;color:#1e40af;border-radius:4px;padding:1px 6px">${ICONsz("package",11)} كتالوج</span>`
        : r.source==="ai"
      ...
      <div style="display:grid;grid-template-columns:2fr 70px 60px 72px 78px 30px;gap:6px;align-items:center;margin-bottom:6px;background:var(--surface2);border:1px solid var(--border);border-radius:8px;padding:6px 8px">
        ...
        <button class="btn btn-ghost btn-sm" style="color:var(--danger);padding:3px 7px;display:inline-flex;align-items:center;justify-content:center" onclick="window.priceAnalysis.removeRow('${kind}',${i})" title="حذف">${ICONsz("trash",13)}</button>
      </div>`;
    }).join("");
  }
  ...
  function refreshPrices(){
    ...
    (_draft.materials||[]).forEach(function(r){
      ...
      /* v18.9oj: التحليل المحفوظ قبل الدمج يحمل مرساةً مدموجة. والحجب وحده كان
         سيُسقط المطابقة صامتاً فيقول «الأسعار محدَّثة بالفعل» وهي بائتة — أسوأ من
         العلّة. فتُحلّ المرساة إلى الوجهة (نفس _catResolveId في النواة، مصدرٌ واحد)
         وتُثبَّت على الصف فلا يُسأل عنها ثانيةً — نمط v18.9nw. */
      const _rid = catResolve(r.refId);
      const it = mats.find(x=>x.id===_rid) || mats.find(x=>x.code&&r.refCode&&x.code===r.refCode);
      if(it && _rid!==r.refId){ r.refId=it.id; r.refCode=it.code||""; r.name=it.name||r.name; }
  ...
  async function aiSuggest(){
    ...
    const mySession = _editorSession;   // بصمة الجلسة الحالية للتحقق بعد await
...
"- أرقام واقعية للسوق السعودي. أعِد JSON صالحاً فقط.";
      ...
      // حارس الهوية: إن أُغلق المحرّر أو فُتح بند آخر أثناء انتظار الذكاء ⇒ تجاهل النتيجة بالكامل
      if(mySession !== _editorSession){ return; }
      let obj;
      ...
      // لا تُعِد ضبط الزر إلا إن كنا ما زلنا في نفس جلسة المحرّر (وإلا فالزر يخص جلسة أخرى)
      if(mySession===_editorSession && btn){ btn.disabled=false; btn.textContent=oldTxt||"🤖 اقتراح المعاملات والأسعار بالذكاء الاصطناعي"; }
    }
  }
