...
function predelivery() {
  ...
  /* ── سقفُ index.html: سقّاطةٌ تنزل ولا تصعد ────────────────────────────────
     ...
     و١٤٠ و٧٠ سطراً — كلٌّ منها بحجم وحدةٍ صغيرة. ولمّا استُخرج مركزُ العمليات
     (٩٧٠ سطراً) صار للمكسب ما يحرسه.

     **سقّاطةٌ في اتجاهين، وهذا مقصود:**
     • تجاوزُ السقف ⇐ سقوطٌ فوريّ. الإضافةُ الكبيرة مكانُها وحدة.
     • النزولُ تحته بفارقٍ معتبَر ⇐ سقوطٌ أيضاً، برسالةٍ تطلب **خفضَ السقف**.
       فمكسبُ استخراجٍ لا يُثبَّت في السقف يُبتلَع صامتاً في الأسابيع التالية،
       وتعودُ الحالُ إلى ما كانت بلا أن يظهر ذلك في أيّ فرق.
     ...
     محادثةٌ صريحة: «لماذا كبر الملفّ؟» أو «ماذا استُخرج؟». وذلك هو الغرض. */
  {
    ...
    /* رُفع من 37200 إلى 37220 — ‏٤ أسطرٍ في `_ICON`: أيقونةُ `projectsGrid` وسطرا شرحٍ
       لهندستها. **إضافةُ مدخلٍ إلى طقمٍ قائمٍ لا ميزةٌ جديدة**، وموضعُها هو الطقمُ
       نفسُه: `_ICON` مصدرُ كلّ أيقونات المنصّة (`_ic` · `workIconSvg` · أيقوناتُ
       المشاريع)، ونسخُ أيقونةٍ واحدةٍ إلى ملفٍّ جديدٍ يشقّ الطقمَ إلى مصدرَين. */
    /* رُفع من 37220 إلى 37320 — ‏٩٨ سطراً لمعالجةِ بطءِ التحميل، وكلُّها **إصلاحٌ
       لمنطقٍ قائمٍ في موضعه** لا ميزةٌ جديدة، وCLAUDE.md صريحةٌ في منعِ نقلِ منطقٍ
       قائمٍ إلى ملفٍّ جديدٍ بحجّة إصلاحه:
       • `showSplash`/`hideSplash` — الشاشةُ كانت تُحجب ٣ ثوانٍ بمؤقّتٍ أعمى، فصارت
         تُخفى على أوّل لقطةِ بلاغات. تعديلُ دالّةٍ قائمةٍ في مكانها.
       • `_deferBoot` — ترتيبُ تشغيلِ مستمعي Firestore في `enterApp`/`openGlobalPurchases`
         نفسِهما؛ خمسةُ أسطرٍ لجدولةٍ تخدم دالّتين هنا ولا معنى لها خارجَهما.
       • `_warnSyncCap` — حارسُ سقفٍ للاستعلامات الثلاثة غيرِ المحدودة، وموضعُه بجوارها.
    ...
    /* ثم رُفع من 37368 إلى 37383 — ‏١٤ سطراً لسداد المالية فوق المتبقّي بتحذيرٍ
    ...
    /* ثم رُفع من 37787 إلى 37817 — ‏٣٠ سطراً لتوليد اسم خطة PPM تلقائياً
       (طلب المالك): الدالة النقية `ppmAutoPlanName` وخريطة صفات التكرار
       و`ppmNameAutoFill` ووصلُها بنافذة الإضافة القائمة. **تعديلُ سلوكِ حقلٍ
    ...
    T("★ والسقفُ ملتصقٌ بالواقع (مكسبُ الاستخراج يُثبَّت فلا يُبتلَع لاحقاً)",
      idxLines > IDX_CEILING - IDX_SLACK,
      `${idxLines} سطراً والسقف ${IDX_CEILING} — اخفِض IDX_CEILING إلى ${idxLines} في hail-tests.js`);
  }
  ...
  // يسهل «تبسيطُها» لاحقاً بحسن نيّةٍ فتنهار المنظومةُ بلا رسالةِ خطأٍ واحدة.
  {
    ...
    // ويُحسَم الانتماءُ بالسؤال عن **قائمة حاويات المشروع** لا عن حقلٍ في الوصف:
    // الصياغةُ الأولى سألت `project_number` — حقلاً لا تعرضه الأداة — فرجع فارغاً
    // **وأعلن سقوطاً كاذباً على إعدادٍ سليم**. فحصٌ يقرأ اسمَ حقلٍ يحكم بما لا يعرف.
    T("★★ st: انتماءُ حاوية التصدير يُحسَم بقائمة حاويات المشروع لا بحقلٍ في الوصف",
      FCHK.includes("buckets list --project=") && !FCHK.includes("value(project_number)"));
    T("★ st: قائمةٌ فارغةٌ تُعامَل سقوطاً لا نجاحاً (لا حكمَ بلا معرفة)",
      /if \[ -z "\$SRC_BUCKETS" \]/.test(FCHK));
    ...
    // ويُبحَث عنه **بسردٍ صريحٍ وgrep** لا بنمطِ `**`: دلالةُ `**` في أنماط gcloud
    // تعبر المجلّدات ولا تُكمل جزءَ اسمِ ملفّ — فأعلنت نقصاً في تصديرٍ مكتمل.
    // نفسُ درسِ `project_number`: الاتّكاءُ على دلالةٍ خاصّةٍ بالأداة يحكم بما لا يعرف.
    // ويُقرأ بالشفرة لا بالشرح: التعليقُ نفسُه يذكر النمطَ المعطوب ليشرحه —
    // وحارسٌ يكسره توثيقُ سببه حارسٌ يُغري بحذف التوثيق (الدرسُ نفسُه مرّتين).
    T("★ st: فحصُ الصحّة يشترط ملفَّ الفهرس (المنقطعُ يبدو سليماً في القائمة)",
      FCHK.includes("overall_export_metadata") &&
      !/\*\*overall_export_metadata/.test(code(FCHK)) &&
      /grep -c 'overall_export_metadata'/.test(code(FCHK)));
    ...
    // ★★ والفرزُ وحدَه لا يكفي: ختمُ البروفات أنشأ `_drills/` في الحاوية نفسِها،
    //    و`_` **يسبق الأرقام في الفرز التنازليّ** — فصار «أحدثُ تصدير» يشير إلى
    //    مجلّد الختم في **الفحص والبروفة معاً**. ميزةُ الرقابة كسرت كاشفَ التصدير،
    //    وهو أخطرُ صنفٍ: تغييرٌ في مكانٍ يُسقط قارئاً في مكانٍ آخر بلا صلةٍ ظاهرة.
    //    فالترشيحُ **بشكلِ مجلّد التصدير** يجعل الكاشفَ لا يبالي بما يُضاف حولَه.
    T("★★ st: كاشفُ أحدث تصديرٍ يُرشِّح شكلَ المجلّد فلا يلتقط `_drills/`",
      [FDRL, FCHK].every(s =>
        /grep -E '\/\[0-9\]\{4\}-\[0-9\]\{2\}-\[0-9\]\{2\}T/.test(s)));
...
function inventoryReportsTests() {
  ...
  /* ══ ★★ v18.9.2741: ورقةُ التقرير **طوليّة** — والهندسةُ كانت كذلك دائماً ══
     ...
     وكان الإعلانُ `size:A4 landscape` — ورقةً عرضُها ٢٩٧مم — فبقيت الصورُ على عرضها
     الطوليِّ ملتصقةً بحافةٍ واحدةٍ ونحوُ ٩٥مم بياضاً في الأخرى. **الأرقامُ لم تكن
     خاطئةً، الورقةُ كانت.** والحارسُ يربط الأمرين فلا ينفصلا ثانيةً. */
  {
    const ivr = IVR_PATH ? fs.readFileSync(IVR_PATH, "utf8") : "";
    const ctr = CTR_PATH ? fs.readFileSync(CTR_PATH, "utf8") : "";
    T("★★ pg: لا إعلانَ `@page` عرضيّاً في تقارير المخزون (الورقتان: الرسميةُ والاحتياطية)",
      !!ivr && !/@page\{size:[^}]*landscape/.test(ivr));
    T("★★ pg: والإعلانان طوليّان صراحةً",
      /@page\{size:A4;margin:3mm 0 11\.5mm\}/.test(ivr) && /@page\{size:A4;margin:12mm\}/.test(ivr));
    /* ★★★ الحارسُ الحقيقيّ: مقاساتُ الترويسة هنا = مقاساتُها في `contracts.js`.
       فلو غُيِّر الاتجاهُ يوماً بلا إعادةِ حسابِ المقاسات سقط هذا فوراً — وهو
       الرباطُ الذي غيابُه أنتج العطل. */
    if (ivr && ctr) {
      const geo = src => (src.match(/\.lh-h\{[^}]*\}|\.lh-f\{[^}]*\}|\.lh-m\{[^}]*\}|\.sp-h\{[^}]*\}\.sp-f\{[^}]*\}/g) || []).join("|");
      T("★★★ pg: وهندستُها مطابقةٌ لورقة التعاقدات الطوليّة حرفاً بحرف (مصدرُ المقاسات واحد)",
        geo(ivr) === geo(ctr) && geo(ivr).length > 0, geo(ivr).slice(0, 90));
    }
    /* ── والطوليُّ يعطي ١٨٦مم بدل ٢٧٣مم: أعرضُ تقريرٍ أحدَ عشرَ عموداً ──
       وبلا عرضٍ صريحٍ يوزّع المتصفّحُ بحسب أطول محتوى فيخرج الباقي عن الورقة —
       **والخارجُ عن الورقة يُقصّ في الطباعة بلا رسالة.** */
    T("★★ pg: أعمدةُ الورقة بنسبٍ صريحةٍ و`table-layout:fixed` (لا عرضٌ يقرّره المحتوى)",
      /function _pdfColGroup\(cols\)\{/.test(ivr) &&
      /\.tbl\{width:100%;table-layout:fixed/.test(ivr) &&
      /\$\{_pdfColGroup\(rep\.cols\)\}<thead>/.test(ivr));
    T("★★ pg: والنسبُ مشتقّةٌ من `c.w` نفسِها التي تبني أعمدةَ Excel (مصدرٌ واحد)",
      /\(_num\(c&&c\.w\)\|\|16\)/.test(ivr) && /wch:c\.w\|\|16/.test(ivr));
    T("★ pg: وعشرةُ أعمدةٍ فأكثر تُكثَّف بدل أن تُقصّ",
      /rep\.cols\.length>=10\?" dense":""/.test(ivr) && /\.tbl\.dense\{font-size:9px\}/.test(ivr));
    T("★ pg: وترويسةُ الجدول تلتفّ فلا تفيض على ورقةٍ أضيق",
      /\.tbl thead th\{[^}]*word-break:break-word/.test(ivr));
  }
  ...
  /* ════ لا حسابَ قبل وصول لقطة الأرصدة ════
     كلُّ رقمٍ في الوحدة يبدأ من الرصيد الحاليّ ثم يرجع بالحركات. فقبل أوّل لقطةٍ
     لـ`global_inventory` تكون الأرصدةُ فارغةً، فتُبنى الصفوفُ من السجلّ وحدَه ويخرج
     **تقريرٌ كاملُ الشكل كاذبُ الأرقام** (افتتاحيٌّ سالبٌ وختاميٌّ صفر) بلا رسالةِ
     خطأٍ في أيّ مكان. وسلوكُ الحارس يفحصه **سيناريو ١٥** في متصفّحٍ حقيقيّ؛ وهذه
     حرّاسُ بنيةٍ مساندةٌ لا بديلة. */
  T("★ الوحدة تفحص علَم وصول اللقطة (_fsLoaded.inventory) قبل الحساب",
    /window\._fsLoaded\.inventory/.test(src));
  T("★ التوليد يعود مبكّراً قبل كتابة _out عند غياب اللقطة",
    /if\(!_invReady\(\)\)\{[\s\S]{0,400}?return;/.test(src));
  T("والمزامنة تُستدعى قبل الرفض (دالّتُها idempotent كزرّ التحديث)",
    /startInventorySync\(\)/.test(src));
  ...
  /* ════ الوارد اليدويّ في إعادة حساب الرصيد (index.html) ════
     ...
     يُحرَس **تنفيذاً** بسيناريو ١٤ (تُشغَّل إعادةُ الحساب ثمّ يُطابَق ختاميُّ
     التقرير بالرصيد المكتوب)، وهذا حارسُ بنيةٍ يمنع رجوعَ الشرط إلى `"in"` وحدَه. */
  T("★★ recalcInventoryFromLog تقرأ manual_in مع in (وإلا تلاشى المخزونُ اليدويّ)",
    /if\(t==="in" \|\| t==="manual_in"\)\{/.test(HTML),
    'الشرطُ ما زال `if(t==="in"){` — إعادةُ الحساب تُسقط الوارد اليدويّ');
...
function financialInvariants() {
  ...
  T("★ تجاوزُ المتبقّي في سداد المالية يُؤكَّد (confirm) بدل المنع — مع تحذيرٍ حيٍّ تحت الحقل",
    /if\(amount > remaining \+ 0\.01\)\{[\s\S]{0,500}confirm\([\s\S]{0,300}يتجاوز المتبقّي/.test(HTML) &&
    HTML.includes('id="fp-over-warn"') && HTML.includes("_fpOverWarn(") &&
    !/if\(amount > remaining \+ 0\.01\)\{ toast\(/.test(HTML));
  ...
  if (CTR_PATH) {
           ...
           لكلّ مشرفٍ وفنيٍّ قائمٍ بأثرٍ رجعيّ لمجرّد أنّ خانتَه كانت مؤشَّرة. */
    {
      ...
      if(!CT || typeof CT.canView !== "function"){
        ...
        /* ══ ★★★ v18.9.2751: الآيبانُ — المضيفُ يكتبه **عند الإنشاء وحدَه** ══
        ...
        /* ★★★ v18.9.2751: والماليةُ كذلك — `vendorUpdateOk` تسمح لها على الخادم منذ
...
function supervisorReceiptGuards() {
  ...
  /* ══ وثيقةُ المحضر — الورقةُ التي تُوقَّع (طلب المالك 31/08) ══
     ...
     قائمٌ بذاته يُحتكم إليه عند الخلاف — فيحتاج ورقةً يوقّعها من استلم. */
  T("★ وثيقةُ المحضر معروضةٌ وقابلةٌ للفحص بلا نافذة طباعة",
    typeof S.printReceipt === "function" && typeof S.paperHTML === "function" &&
  ...
  /* **أهمُّ حارسٍ فيها:** الورقةُ لقطةُ لحظتها. لو حُسب التراكميُّ والمتبقّي من
     ...
     فيكون الموقِّعُ قد وقّع على غير ما تُظهره الورقة. */
  const recs2 = [{ items: [{ idx: 0, qty: 2 }] }, { items: [{ idx: 0, qty: 3 }] }];
  T("★★ التراكميُّ يقف عند المحضر المطبوع لا عند حالة اليوم",
    (S._cumUpTo(recs2, 0)[0] === 2) && (S._cumUpTo(recs2, 1)[0] === 5) &&
    JSON.stringify(S._cumUpTo(recs2, -1)) === "{}",
    JSON.stringify([S._cumUpTo(recs2, 0), S._cumUpTo(recs2, 1)]));

  const poDoc = { id: "PO-9", projectName: "مشروع حائل", building: "المبنى", vendor: "مورد أ",
    items: [{ itemName: "جلبة", unit: "قطعة", qty: 5 }, { itemName: "مطب", unit: "متر", qty: 10 }],
    svReceipts: [
      { ref: "SVR-01", by: "أسامة", at: "2026-08-31T10:00:00Z", notes: "حالة جيدة",
        photos: [{ url: "u1" }], items: [{ idx: 0, qty: 2 }] },
      { ref: "SVR-02", by: "أسامة", at: "2026-09-01T10:00:00Z", photos: [],
        items: [{ idx: 0, qty: 3 }, { idx: 1, qty: 10 }] },
    ] };
  const paper1 = S.paperHTML(poDoc, "SVR-01"), paper2 = S.paperHTML(poDoc, "SVR-02");
  T("★★ ورقةُ المحضر الأوّل تُظهر أرقامَ لحظته لا حصيلةَ الطلب اليوم",
    /استلامٌ جزئيّ/.test(paper1) && /اكتمل الاستلام/.test(paper2));
  T("★★ وفيها إقرارٌ منصوصٌ فوق التوقيع — لا خانةُ توقيعٍ صامتة",
    /أقرُّ بأنّ الكمياتِ/.test(paper1) &&
    /المستلم الميدانيّ \(المشرف\)/.test(paper1) && /مسؤول المستودع/.test(paper1) &&
    /مدير المشروع/.test(paper1));
  T("★ وتُعلن حدَّها: توثيقٌ ميدانيٌّ لا حركةُ مخزون (لا تُقرأ سنداً للرصيد)",
    /توثيقٌ ميدانيّ<\/b> لا حركةَ مخزون/.test(paper1) && /GRN/.test(paper1));
  T("★ وغيابُ الصور يُعلَن في الورقة لا يُسكَت عنه",
...
/* ════════════════════════════════════════════════════════════════════
   ...
   عنده غير صفحة الموارد البشرية». والجذر: تلك المفاتيحُ تحكم صفحاتِ **وضع
   المشروع**، ودورُه من أدوار الوضع المركزيّ التي لا ترى شاشةَ مشروعٍ أصلاً —
   فخاناتُها زينةٌ بلا أثر. والعلاج شقّان: لا تُعرض خانةٌ لا أثرَ لها، ويُضاف
   مفتاحٌ **مانحٌ** واحدٌ يعمل فعلاً: «طلب شراء».
   الحارس يُنفّذ الدوالَّ على مستخدمين حقيقيين — لا يقرأ سطورَها.
   ════════════════════════════════════════════════════════════════════ */
function hrPurchaseRequestGuards() {
  H("صلاحية «طلب شراء» لمسؤول الموارد البشرية");
  ...
  T("★ hrpo: كلُّ دوالّ البوّابة موجودةٌ في index.html", absent.length === 0, absent.join(","));
  ...
  const hrNo  = { role:"hr_officer", name:"عبدالله آدم", permissions:{ ...basePerms } };
  const hrYes = { role:"hr_officer", name:"عبدالله آدم", permissions:{ ...basePerms, poRequest:true } };
  ...
  T("★★ hrpo: بلا المفتاح — مسارُ الشراء كلُّه محجوبٌ عن showPage (لا بابَ بالعنوان)",
    ALL_PO.every(p => B(hrNo).includes(p)), B(hrNo).join(","));
  T("★★ hrpo: بالمفتاح — يُفتح بابان اثنان فقط: الإنشاءُ والمتابعة",
    !B(hrYes).includes("purchases") && !B(hrYes).includes("new-purchase"),
  ...
  T("★★ hrpo: الحفظُ يُبقي المفاتيحَ غيرَ المعروضة كما هي (لا محوَ ما لم يُرَ)",
    /const shown=new Set\(_permKeysForUser\(USERS\[idx\]\)\);/.test(HTML) &&
    /if\(!shown\.has\(k\)\)\{ if\(k in prev\) perms\[k\]=prev\[k\]; return; \}/.test(HTML));

  // ── (٧) السايدبار: القاعدةُ المانحةُ أعلى وزناً، والصنفُ يُرفع ويُنزع ──
  T("★ hrpo: قاعدةُ CSS تفتح مجموعةَ الشراء بصنفين على body (تغلب قاعدةَ الإخفاء)",
    /body\.role-hr-officer\.perm-po-request #hdr-grp-po\{display:flex!important\}/.test(HTML) &&
    /body\.role-hr-officer\.perm-po-request #grp-po\{display:block!important\}/.test(HTML));
  T("★ hrpo: وأزرارُ التقارير والتدقيق والرقابة والمستعاض تبقى مخفيةً عنه",
    ["purchase-reports","substitute-budget","po-audit","finance-audit"].every(p =>
      new RegExp('body\\.role-hr-officer #grp-po \\.sidebar-nav-btn\\[data-page="' + p + '"\\]').test(HTML)));
...
function vendorPOIssuance() {
  ...
  /* ── الاعتمادات الداخلية أسفل الوثيقة (طلب المالك — نمط ورقة المستخلص) ──
     تُشتقّ من سجلّ الطلب: البوّابةُ تُعرَّف بالحالة التي تُغادَر قُدُماً، وآخرُ
     عبورٍ يغلب (إرجاعٌ ثم إعادةُ اعتماد)، وللتنفيذيّ ختمُه المقرونُ بمبلغه
     وللسداد مستندُ الدفع، وبوّابةُ التنفيذي شرطيةٌ لا تُطبَع لطلبٍ لم يمرّ بها. */
  T("تعرّض _signoffs", typeof V._signoffs === "function");
  if (typeof V._signoffs === "function") {
    const tlPO = {
      id: "PO-X", status: "proc_executing",
      timeline: [
        { code: "pending_pm",      by: "النظام",        at: "2026-08-01T08:00:00Z" },
        { code: "pm_approved",     by: "مدير المشاريع", at: "2026-08-01T09:00:00Z" },
        { code: "wh_review",       by: "النظام",        at: "2026-08-01T09:00:01Z" },
        { code: "wh_reviewed",     by: "أمين المستودع", at: "2026-08-01T10:00:00Z" },
        { code: "pending_proc",    by: "النظام",        at: "2026-08-01T10:00:01Z" },
        { code: "pending_ceo",     by: "مسؤول المشتريات", at: "2026-08-01T11:00:00Z" },
        { code: "pending_finance", by: "التنفيذي",      at: "2026-08-01T12:00:00Z" },
        { code: "vendor_po_issued", by: "مسؤول المشتريات", at: "2026-08-01T12:30:00Z" }, // قيدُ الوحدة لا انتقال
        { code: "proc_executing",  by: "المالية",       at: "2026-08-01T13:00:00Z" },
      ],
      ceoApprovedBy: "المدير التنفيذي", ceoApprovedAt: "2026-08-01T12:00:00Z",
      payment: { paidBy: "المالية", paidAt: "2026-08-01T13:00:00Z" },
    };
    const sg = V._signoffs(tlPO);
    const by = k => (sg.find(g => g.key === k) || {}).by;
    T("★ البوّابات الخمس من السجل الفعلي: مدير المشاريع · المستودع · المشتريات · التنفيذي · السداد",
      by("pm") === "مدير المشاريع" && by("wh") === "أمين المستودع" && by("proc") === "مسؤول المشتريات" &&
      by("ceo") === "المدير التنفيذي" && by("fin") === "المالية",
      JSON.stringify(sg.map(g => g.key + ":" + (g.by || "؟"))));
    T("★ قيد vendor_po_issued لا يكسر تتبّع الانتقالات", by("fin") === "المالية");
    // طلبٌ دون العتبة: لا صندوقَ للتنفيذي أصلاً — والبوّابةُ غيرُ المعتمَدة تُطبَع «لم يعتمد بعد»
    const sg2 = V._signoffs({ id: "PO-Y", status: "pending_proc", timeline: [
      { code: "pm_approved", by: "مدير", at: "2026-08-02T08:00:00Z" },
    ] });
    T("★ التنفيذي شرطيٌّ (لا يُطبَع لطلبٍ لم يمرّ به) والبوّاباتُ غيرُ الموقَّعة فارغةٌ صراحةً",
      !sg2.some(g => g.key === "ceo") && sg2.find(g => g.key === "proc").by === "" &&
      sg2.find(g => g.key === "pm").by === "مدير");
    // إرجاعُ الأدمن ثم إعادةُ الاعتماد — آخرُ عبورٍ يغلب
    const sg3 = V._signoffs({ id: "PO-Z", status: "proc_executing", timeline: [
      { code: "pm_approved",  by: "الأول",  at: "2026-08-03T08:00:00Z" },
      { code: "pending_pm",   by: "الأدمن", at: "2026-08-03T09:00:00Z" },
      { code: "pm_approved",  by: "الثاني", at: "2026-08-03T10:00:00Z" },
    ] });
    T("★ بعد الإرجاع وإعادة الاعتماد يُقرأ الاعتمادُ الأخير", sg3.find(g => g.key === "pm").by === "الثاني");
    // قرار المالك (قبل دمج #352): «مدير النظام» يُعرَض «مدير المشاريع» في الخانات —
    // حساب الأدمن يقوم بدور مدير المشاريع، والوثيقة للمورد تُظهر الصفة لا اسم الحساب.
    const sg4 = V._signoffs({ id: "PO-A", status: "proc_executing", timeline: [
      { code: "pm_approved", by: "مدير النظام", at: "2026-08-04T08:00:00Z" },
    ] });
    T("★ «مدير النظام» في الخانات يُعرَض «مدير المشاريع» (قرار المالك)",
      sg4.find(g => g.key === "pm").by === "مدير المشاريع",
      sg4.find(g => g.key === "pm").by);
    T("★ القسم يُطبَع في الوثيقة (الاعتمادات الداخلية + «لم يعتمد بعد»)",
  ...
  /* ── الشريطُ الأزرق أسفل الترويسة (بلاغ المالك الثالث) ──
     الورقةُ الرسمية جدولٌ (`.pg`) بـthead يحجز مساحةَ الترويسة وحاشيتُه شفافة —
     فقاعدةٌ **عامة** على `table thead` تلوّن صفَّه شريطاً كحلياً أسفل الترويسة
     **على كل صفحة** (الـthead يتكرّر). أنماطُ الجدول تُحصَر في جدول البنود. */
  T("★ لا قاعدةَ جدولٍ عامةً في المطبوع — كلُّها محصورةٌ في .items",
    src.includes('<table class="items">') && src.includes(".items thead tr{background:#1b3a6b") &&
    !/'table(?: th| thead|\{)[^']*'/.test(src.slice(src.indexOf("baseCss"))));
  T("★ بطاقاتُ المعلومات ٤ أعمدة مضغوطة (كانت ٣ بثلاثة صفوفٍ ترحّل التوقيعات)",
    src.includes(".ig{display:grid;grid-template-columns:repeat(4,1fr)"));
...
/* ════════════════════════════════════════════════════════════════════
   خطط PPM: توليد اسم الخطة تلقائياً (طلب المالك) —
   «خطة الصيانة الوقائية» + صفة التكرار + اسم الأصل المختار.
   الدالة النقية ppmAutoPlanName تُنفَّذ هنا فعلاً، والتوصيل يُحرس نصّياً.
   ════════════════════════════════════════════════════════════════════ */
function ppmAutoNameGuards() {
  H("خطط PPM — توليد اسم الخطة تلقائياً");

  // ── (١) الدالة النقية: تُستخرج من index.html وتُنفَّذ فعلاً ──
  const adj = HTML.match(/const PPM_FREQ_ADJ = \{[^\n]*\};/);
  const m = HTML.match(/function ppmAutoPlanName\(freq, assetName\)\{[\s\S]*?\n\}/);
  T("ppmAutoPlanName موجودة (دالة نقية) مع خريطة صفات التكرار", !!m && !!adj);
  if (m && adj) {
    const fn = new Function(adj[0] + m[0] + "; return ppmAutoPlanName;")();
    T("★★ الاسم = «خطة الصيانة الوقائية» + صفة التكرار + اسم الأصل",
      fn("ربع سنوي", "وحدة تكييف (Split)") === "خطة الصيانة الوقائية الربع سنوية - وحدة تكييف (Split)");
    T("★ بلا أصل: يقف الاسم عند صفة التكرار",
      fn("شهري", "") === "خطة الصيانة الوقائية الشهرية" &&
      fn("شهري", null) === "خطة الصيانة الوقائية الشهرية");
    T("★ كل تكرارات القائمة الستة لها صفة معرَّفة بأداة التعريف",
      ["يومي", "أسبوعي", "شهري", "ربع سنوي", "نصف سنوي", "سنوي"]
        .every(f => fn(f, "").startsWith("خطة الصيانة الوقائية ال")));
    T("تكرار خارج الخريطة يمرّ كما هو ولا يكسر التوليد",
      fn("كل ساعتين", "مضخة") === "خطة الصيانة الوقائية كل ساعتين - مضخة" &&
      fn("", "") === "خطة الصيانة الوقائية");
  }
  ...
  T("★★ التوليد لا يلمس اسماً كتبه المستخدم ولا وضعَ التعديل",
    ...
    /inp\.value\.trim\(\) && inp\.value !== inp\.dataset\.auto/.test(HTML));
  T("★ فتح نافذة الإضافة يمهّد الحقل ويولّد الاسم، وإعادة بناء قائمة الأصول تعيد التوليد",
    /document\.getElementById\("ppm-name"\)\.dataset\.auto="";/.test(HTML) &&
    (HTML.match(/^\s*ppmNameAutoFill\(\);$/gm) || []).length >= 4);
}
