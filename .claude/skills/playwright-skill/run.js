...
/**
 * Fall back to a system-provided Chromium when the Playwright-managed
 * build is absent (e.g. sandboxed environments with a pre-installed
 * browser at /opt/pw-browsers/chromium and downloads blocked).
 */
function patchChromiumExecutableFallback() {
  const FALLBACK = process.env.PW_CHROMIUM_FALLBACK || '/opt/pw-browsers/chromium';
  try {
    const { chromium } = require('playwright');
    let managedExists = false;
    try {
      managedExists = fs.existsSync(chromium.executablePath());
    } catch (e) {
      managedExists = false;
    }
    if (managedExists || !fs.existsSync(FALLBACK)) return;
    const origLaunch = chromium.launch.bind(chromium);
    chromium.launch = (options = {}) =>
      origLaunch({ executablePath: FALLBACK, args: ['--no-sandbox'], ...options });
    const origPersistent = chromium.launchPersistentContext.bind(chromium);
    chromium.launchPersistentContext = (userDataDir, options = {}) =>
      origPersistent(userDataDir, { executablePath: FALLBACK, args: ['--no-sandbox'], ...options });
    console.log(`ℹ️  Using fallback Chromium: ${FALLBACK}`);
  } catch (e) {
    // playwright not installed yet — main() handles installation
  }
}
...
async function main() {
  ...
  // Use pre-installed system Chromium when the managed build is missing
  patchChromiumExecutableFallback();
