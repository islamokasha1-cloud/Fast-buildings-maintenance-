...
(function(){
  ...
  function _canDelete(){ // حذف الجرد — المسؤول (الأدمن) فقط
    try{ return (typeof isAdmin==="function") && isAdmin(); }catch(e){ return false; }
  }
  function _delBtn(t){ // زر حذف صغير — يظهر للأدمن فقط، ويوقف انتشار النقر داخل صفٍّ/بطاقةٍ قابلة للفتح
    if(!_canDelete()) return "";
    return `<button class="btn btn-danger btn-sm" onclick="event.stopPropagation(); window.stocktake._delete('${t.id}')" style="font-size:11px">🗑 حذف</button>`;
  }
  ...
  function _listHtml(){
    ...
    const doneRows = done.slice(0,50).map(t=>{
      ...
      return `<tr style="cursor:pointer" onclick="window.stocktake.open('${t.id}')">
        ...
        ${_admDel?`<td style="padding:9px 12px;text-align:center">${_delBtn(t)}</td>`:""}
      </tr>`;
    }).join("");
        ...
        <div style="overflow-x:auto">
          <table style="width:100%;border-collapse:collapse">
            <thead><tr style="background:var(--surface2)">
              ...
              ${_admDel?`<th style="padding:9px 12px;text-align:center;font-size:12px">حذف</th>`:""}
            </tr></thead>
  ...
  function _detailHtml(t){
    ...
    if(_canDelete()){
      actions += ` <button class="btn btn-danger btn-sm" onclick="window.stocktake._delete('${t.id}')">🗑 حذف الجرد</button>`;
    }
  ...
  // ════════ حذف الجرد (المسؤول فقط) ════════
  // يحذف وثيقة الجرد من Firestore + الملف الموقّع من Storage (أفضل جهد). للجرد المطبَّق:
  // الحذف يُزيل السجل فقط ولا يُلغي تسويات المخزون — الأرصدة وقيود الحركة تبقى كما هي
  // (تاريخ لا يُعاد كتابته). onSnapshot يعيد رسم القائمة بلا ترقيع محلي.
  async function _delete(id){
    const t = _takes.find(x=>x.id===id);
    if(!t){ _toast("⚠ الجرد غير موجود","warn"); return; }
    if(!_canDelete()){ _toast("⚠ صلاحية المسؤول فقط","warn"); return; }
    const applied = (t.status==="applied");
    const msg = applied
      ? "سيُحذف سجل هذا الجرد نهائياً.\n\nمهم: الحذف لا يُلغي تسويات المخزون التي طُبّقت — أرصدة المخزون وحركاته تبقى كما هي. يُزال السجل من تاريخ الجرد فقط."
      : "سيُحذف سجل هذا الجرد نهائياً ولا يمكن استرجاعه.";
    const ok = await _confirm({title:"حذف الجرد: "+(t.warehouseName||""), msg, icon:"🗑", okText:"حذف نهائي", okClass:"btn-danger"});
    if(!ok) return;
    try{
      if(t.signedFilePath && typeof storage!=="undefined" && storage){ try{ await storage.ref(t.signedFilePath).delete(); }catch(_e){} }
      await db.collection(COLL()).doc(t.id).delete();
      _audit("حذف جرد","المستودع: "+(t.warehouseName||"")+" — الحالة: "+(STATUS[t.status]||t.status));
      _toast("🗑 حُذف الجرد","success");
      if(_curId===t.id) back();
      // onSnapshot يعيد الرسم — لا ترقيع محلي
    }catch(e){ console.warn("delete take:",e); _toast("⚠ تعذّر الحذف: "+((e&&e.message)||""),"warn"); }
  }
