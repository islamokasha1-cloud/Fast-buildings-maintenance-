...
(function(){
  ...
  function _detailHtml(t){
    ...
    if(applied || t.status==="pending_approval"){
      actions += ` <button class="btn btn-ghost btn-sm" onclick="window.stocktake.print()">🖨 تقرير</button>`;
    }
  ...
  function print(){
    ...
    const rowsHtml=(t.snapshot||[]).map(s=>{
      ...
      return `<tr>
        <td style="padding:6px 8px;border:1px solid #ddd">${_esc(s.itemName)}${s.itemCode?` <span style="color:#888">(${_esc(s.itemCode)})</span>`:""}</td>
        <td style="padding:6px 8px;border:1px solid #ddd;text-align:center">${_fmt(s.systemQty)} ${_esc(s.unit||"")}</td>
        <td style="padding:6px 8px;border:1px solid #ddd;text-align:center">${has?_fmt(cnt):"—"}</td>
        <td style="padding:6px 8px;border:1px solid #ddd;text-align:center;color:${dCol};font-weight:700">${dTxt}</td>
      </tr>`;
    }).join("");
    const html=`
      <div style="font-family:'Segoe UI',Tahoma,sans-serif;direction:rtl;padding:10px">
        <h2 style="margin:0 0 4px">تقرير جرد شهري — ${_esc(t.warehouseName)}</h2>
        <div style="color:#555;font-size:13px;margin-bottom:10px">
          الحالة: ${_esc(STATUS[t.status]||t.status)} — بدأه: ${_esc(t.createdBy)} (${_esc(_shortDate(t.createdAt))})
          ${t.status==="applied"?` — اعتمده: ${_esc(t.approvedBy||"—")} (${_esc(_shortDate(t.appliedAt))})`:""}
        </div>
        <div style="font-size:13px;margin-bottom:10px">
          إجمالي البنود: ${sm.total} — عُدّ: ${sm.counted} — زيادة: ${sm.surplus} — عجز: ${sm.shortage} — مطابق: ${sm.matched}${sm.bigCount?` — فروقات كبيرة: ${sm.bigCount}`:""}
        </div>
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          <thead><tr style="background:#f3f4f6">
            <th style="padding:6px 8px;border:1px solid #ddd;text-align:right">البند</th>
            <th style="padding:6px 8px;border:1px solid #ddd">رصيد النظام</th>
            <th style="padding:6px 8px;border:1px solid #ddd">المعدود</th>
            <th style="padding:6px 8px;border:1px solid #ddd">الفرق</th>
          </tr></thead>
