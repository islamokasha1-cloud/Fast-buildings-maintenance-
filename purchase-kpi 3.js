...
const STAGE_ORDER = [
  ...
  // القائمة. وإضافتها وحدها لا تُنتج شيئاً اليوم: القائمة كلها بلا وقود لأن
  // timeline[].event نصٌّ حرّ لا رمز (انظر timelineReadable أدناه). تُضاف لأنها
  // صحيحة، وتعمل من نفسها لحظة تُكتب الرموز.
...
function computeKPIs(list){
  ...
  /* ══ v1.2: هل المسار مقروء أصلاً؟ ══
     analyzeOrder تفترض أن timeline[].event رمزُ حالةٍ أو تسميتها العربية حرفياً.
     والملف الرئيسي يكتب في كل مواضع timeline.push نصاً حراً وصفياً («تغيير
     الحالة: كذا ← كذا»، «تدقيق استلام — سند GRN-… — …»). فلا يُحلّ أي حدث إلى
     رمز: stageDur تبقى {}، و rejected تبقى [] ولو رُفض الطلب ثلاث مرات — فتقول
     البطاقة «اعتماد من أول مرة: 100%» خضراءَ أبداً، وأعمدةُ الرفض أصفار أبداً.
     وبطاقةٌ تكذب بثقة أسوأ من بطاقةٍ فارغة.

     الكشف بالاشتقاق لا بعلمٍ محفوظ: إن لم يُحلّ ولا حدثٌ واحد في كل الطلبات
     فالمسار غير مقروء بنيوياً ⇒ لا تُعرض أرقامٌ مشتقّة منه. تصحّح نفسها،
     وتُضيء وحدها لحظة تُكتب الرموز — بلا تنظيف ولا هجرة.

     والعلاج الجذري (code على كل timeline.push + قراءة t.code || event) مؤجَّل
     ومقيَّد في «ما تبقّى»: عيبٌ أقدم من pending_extra ولا صلة له بجذر §٣.٦. */
  const timelineReadable = (A.length===0) || Object.keys(stageAgg).length>0 || anyRejected>0;
  ...
  return {
    ...
    stageAvg, timelineReadable,
    rejBy:       timelineReadable ? rejBy : null,
    anyRejected: timelineReadable ? anyRejected : null,
    firstPassRate: (timelineReadable && A.length) ? (A.filter(a=>a.firstPass).length/A.length)*100 : null,
...
function render(){
  ...
  <div class="pkpi-grid">
    ...
    <div class="pkpi-card ${!K.timelineReadable?"":K.firstPassRate>=80?"good":K.firstPassRate<50?"bad":"warn"}"><div class="v">${pct(K.firstPassRate)}</div><div class="l">اعتماد من أول مرة</div><div class="s">${K.timelineReadable?`${nf(K.anyRejected)} طلباً واجه رفضاً واحداً على الأقل`:"يتعذّر القياس — أحداث المسار نصّية لا رموز"}</div></div>
  ...
  <div class="pkpi-charts">
    ${K.timelineReadable?`<div class="pkpi-chart-card"><h4>⏱ متوسط زمن كل مرحلة (يوم) — عنق الزجاجة</h4><div class="cwrap"><canvas id="pkpi-c-stages"></canvas></div></div>`:""}
    ...
    ${K.timelineReadable?`<div class="pkpi-chart-card"><h4>⛔ الرفض حسب جهة الاعتماد</h4><div class="cwrap"><canvas id="pkpi-c-reject"></canvas></div></div>`:""}
...
function drawCharts(K){
  ...
  /* 4) الرفض حسب الجهة */
  ctx = $("pkpi-c-reject");
  if(ctx && K.timelineReadable && K.rejBy){
