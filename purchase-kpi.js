...
function projectOptions(){
  const seen=new Map();
  (window.purchases||[]).forEach(p=>{ if(p.projectId && !seen.has(p.projectId)) seen.set(p.projectId,p.projectName||p.projectId); });
  return [...seen.entries()];
}
...
function filteredList(){
  ...
  let list = (window.purchases||[]).filter(p=>normStatus(p.status)!=="deleted");
