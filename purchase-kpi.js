...
function computeKPIs(list){
  ...
  /* الإنفاق حسب المورد — v18.9tc: **المغلقة فقط بالتكلفة الفعلية**، فيتطابق إجمالي المخطّط مع
     «إجمالي المبالغ المغلقة» في اللوحة و«الإنفاق الفعلي» في المؤشرات (لا يشمل تقديرَ المفتوحة
     بعد أن كان يضخّم الإجمالي — 40,288 مقابل 6,700 الفعلي). المورّد يُقرأ من المورّد **الفعلي**:
       • الطلب المغلق بسندات: يُوزَّع على مورّديه الفعليين بحصصهم الفعلية (poVendorBreakdown).
       • مغلقٌ بلا سندات: تُنسب تكلفتُه الفعلية (actualCost ثم getPOTotal) لمورّده من أي حقل،
         وحتى إن غاب الاسم يُجمَع تحت «غير محدد» بدل أن يُسقَط الإنفاق. */
  const byVendor = {};
  list.forEach(p=>{
    if(!p || !_kpiClosed(p)) return;   // المغلقة فقط — اتساقاً مع بقية أرقام الإنفاق
    let parts=null;
