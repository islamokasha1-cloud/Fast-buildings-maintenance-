...
self.addEventListener('install', event => {
  ...
  self.skipWaiting(); // ← يجبر التفعيل الفوري بدون انتظار
});
...
self.addEventListener('activate', event => {
  ...
  self.clients.claim(); // ← يأخذ التحكم فوراً من كل التبويبات المفتوحة
});

// ── الاعتراض: Network-First للـ HTML، Cache-First لباقي الموارد ──
self.addEventListener('fetch', event => {
  ...
  // Network-First لملف HTML الرئيسي لضمان التحديث دائماً
  const isHTML = url.endsWith('/') || url.endsWith('.html') || url.includes('index.html');
