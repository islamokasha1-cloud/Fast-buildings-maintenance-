...
(function(){
  ...
  function buildDOM(){
    ...
    <div class="card" style="margin-bottom:14px">
      <div class="card-header">
        <div class="card-title">🛠️ كتالوج المصنعيات وأسعار البنود</div>
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
          <button class="btn btn-ghost btn-sm" data-viewer-hide onclick="window.laborCatalog.exportExcel()" title="تصدير Excel">📥 تصدير Excel</button>
          <button class="btn btn-ghost btn-sm" onclick="window.laborCatalog.downloadTemplate()" title="تحميل قالب الاستيراد">📋 قالب الاستيراد</button>
          <button class="btn btn-ghost btn-sm" onclick="window.laborCatalog.triggerImport()" title="استيراد من Excel">📤 استيراد Excel</button>
          <input type="file" id="lab-import-file" accept=".xlsx,.xls" style="display:none" onchange="window.laborCatalog.importExcel(this)">
          <button class="btn btn-primary btn-sm" onclick="window.laborCatalog.openItemModal()">➕ إضافة مصنعية</button>
        </div>
      </div>
      <div class="card-body">
        ...
        <div class="cat-toolbar">
          ...
          <input class="form-input" id="lab-filter-cat" list="lab-cat-filter-datalist" placeholder="🏷️ التصنيف..." oninput="pgReset('${PG_KEY}');window.laborCatalog.render()" style="min-width:150px">
          <datalist id="lab-cat-filter-datalist"></datalist>
          ...
          <div class="modal-body">
            ...
            <div class="form-row" style="margin-bottom:10px">
              <div class="form-group">
                ...
                <input class="form-input" id="lab-category" list="lab-cat-datalist" placeholder="اختر أو اكتب تصنيفاً جديداً...">
                <datalist id="lab-cat-datalist"></datalist>
              </div>
              <div class="form-group">
  ...
  // ══ تحديث datalists التصنيفات ══
  function refreshCategoryDatalists(){
    const cats = currentCategories();
    const opts = cats.map(c=>`<option value="${E(c)}">`).join("");
    const dl1 = document.getElementById("lab-cat-datalist");
    const dl2 = document.getElementById("lab-cat-filter-datalist");
    ...
    tbody.innerHTML = pgItems.map(function(it,i){
      ...
      <tr>
        ...
        <td style="text-align:center">
          ${canEdit?`
            <button class="btn btn-ghost btn-sm" onclick="window.laborCatalog.openItemModal('${E(it.id)}')" style="padding:3px 8px;font-size:11px" title="تعديل">✏️</button>
            <button class="btn btn-danger btn-sm" onclick="window.laborCatalog.del('${E(it.id)}','${E((it.name||"").replace(/'/g,"\\'"))}')" style="padding:3px 8px;font-size:11px" title="حذف">🗑</button>
          `:"—"}
        </td>
      </tr>`;
    }).join("");
  ...
  function openItemModal(id){
    ...
    if(id){
      ...
      document.getElementById("lab-category").value = it.category||"";
  ...
  async function save(){
    ...
    const category = document.getElementById("lab-category").value.trim();
